...
suite "proc getObjectFieldNamesAndTypeNames":
  test "simple object":
    ...
    check getEnvConfig(MyApp) == MyApp(
      name: "envconfig",
      version: "v1.0.0",
      dir: "/opt/envconfig",
      port: 1234,
  ...
  test "env prefix is MY_APP":
    ...
    check getEnvConfig(MyApp, "MY_APP") == MyApp(
      name: "envconfig2",
      )
  ...
  test "all types":
    ...
    check getEnvConfig(AllTypes) == AllTypes(
      s: "string",
      i1: 1,
      i2: 127'i8,
      i3: 32767'i16,
  ...
  test "array type":
    ...
    check getEnvConfig(ArrayObj) == ArrayObj(
  ...
  test "RangeError: int8":
    type ErrorObj = object
      v: int8
    putEnv("ERROROBJ_V", "128")
    expect RangeError:
      discard getEnvConfig(ErrorObj)

  test "RangeError: int16":
    type ErrorObj = object
      v: int16
  ...
  test "NoRangeError: uint8":
    type ErrorObj2 = object
      v: uint8
    putEnv("ERROROBJ_V", "256")
    check getEnvConfig(ErrorObj2).v == 0

  test "NoRangeError: uint16":
    type ErrorObj2 = object
      v: uint16
    putEnv("ERROROBJ_V", "65536")
    check getEnvConfig(ErrorObj2).v == 0

  test "NoRangeError: uint32":
    type ErrorObj2 = object
      v: uint32
    putEnv("ERROROBJ_V", "4294967296")
    check getEnvConfig(ErrorObj2).v == 0

  test "NoRangeError: uint64":
    type ErrorObj2 = object
      v: uint64
    putEnv("ERROROBJ_V", "18446744073709551616")
    check getEnvConfig(ErrorObj2).v == 0

  test "env doesn't exist":
    type ReqObj = object
      v: string
    putEnv("reqobj_V", "test")
    expect ValidationError:
      discard getEnvConfig(ReqObj, requires = @["v"])
    ...
    putEnv("REQOBJ3_V1", "")
    expect ValidationError:
      discard getEnvConfig(ReqObj2, requires = @["v2"])

  test "validation: min":
    type ValMinObj = object
      n: int
    putEnv("VALMINOBJ_N", "10")
    check getEnvConfig(ValMinObj, mins = @[("n", 10.0)]).n == 10
    type ValMinObj2 = object
      n: int
    putEnv("VALMINOBJ2_N", "9")
    expect ValidationError:
      discard getEnvConfig(ValMinObj2, mins = @[("n", 10.0)])

  test "validation: max":
    type ValMaxObj = object
      n: int
    putEnv("VALMAXOBJ_N", "100")
    check getEnvConfig(ValMaxObj, maxs = @[("n", 100.0)]).n == 100
    type ValMaxObj2 = object
      n: int
    putEnv("VALMAXOBJ2_N", "101")
    expect ValidationError:
      discard getEnvConfig(ValMaxObj2, maxs = @[("n", 100.0)])

  test "validation: regex pattern":
    type ValRegObj = object
      n: string
    putEnv("VALREGOBJ_N", "bdc")
    check getEnvConfig(ValRegObj, regexps = @[("n", re"[bcd]{3}")]).n == "bdc"
    type ValRegObj2 = object
      n: string
    putEnv("VALREGOBJ2_N", "abc")
    expect ValidationError:
      discard getEnvConfig(ValRegObj2, regexps = @[("n", re"[bcd]{3}")])
  ...
  test "empty val error":
    ...
    # No env
    # putEnv("EMPTYOBJ_I2", "")
    check getEnvConfig(EmptyObj) == EmptyObj()
  ...
  test "ref object":
    ...
    when false:
      ...
      putEnv("REFOBJ_I", "123")
      check getEnvConfig(RefObj)[] == RefObj(s: "hello", i: 123)[]
